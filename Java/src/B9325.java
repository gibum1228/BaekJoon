...
public class B9325 {
	...
	public static void main(String[] args) {
		...
		int size = sc.nextInt(); // 케이스 수
		
		for(int i = 0; i < size; i++) {
			int carPrice = sc.nextInt(); // 자동차 원래 가격
			...
			for(int j = 0; j < option; j++) {
				int optionCount = sc.nextInt(); // 옵션 갯수
				int optionPrice = sc.nextInt(); // 옵션 가격
				
				carPrice += (optionCount * optionPrice); // 총합
			}
