...
int main() {
	int grade[5], result = 0, score, peopleCount = 5;

	for(int i = 0; i < 5; i++) {
		scanf("%d", &grade[i]); // 5명의 성적을 차례대로 입력받기
	}
	for (int j = 0; j < 5; j++) {
		if (grade[j] < 40) { // 40점 미만일 경우
			score = 40; // 무조건 40점
			result += score;
		}
		else { // 40점 이상일 경우
			score = grade[j];
			result += grade[j]; // 점수 그대로 더하기
		}
	}

	printf("%d", result / peopleCount); // 총 평균값 출력
